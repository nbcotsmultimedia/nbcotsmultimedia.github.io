...
function loadData() {
	...
	Papa.parse("https://docs.google.com/spreadsheets/d/e/2PACX-1vQG2HTpqILQkj1zQC7qlxFr_cHhDGRTqDOBFSNy4E02FeLe6lH0iP5o1SBPFZqPkRPCHvY3Q15p1DYI/pub?gid=0&single=true&output=csv", {
		download: true,
    header: true,
		config,
...
function createGrid() {
	...
	for (i=0; i<totalEntries; i++) {
		...
		$("#thumbsCon").append("<div class='thumb' id='c" + i + "' data-id='" + i + "'>" +
													 "<div class='thMask' id='i" + i + "'><img src='" + tmpImg[0] + "?quality=85&strip=all&w=100&h=150&crop=1&fit=125%2C125' class='img-fluid' /></div>" +
													 "<p class='thLast'>" + allData[i].name + "</p>" +
													 "</div>");
...
function loadCard(which) {
	...
	$("#cardPic").html("<img src='" + tmpImg[0] + "?quality=85&strip=all&w=500&h=750&crop=1&fit=650%2C650' class='img-fluid' />");
	$("#cardName").html(allData[which].name);
	$("#cardCaption").html("<span class='countDisp'>" + theCount + "/" + totalEntries + "</span>&nbsp;&nbsp;&nbsp;" + allData[which].caption);

...
function loadData() {
	...
	Papa.parse('https://docs.google.com/spreadsheets/d/e/2PACX-1vQQQiG4gY7nR57B5jOu8-ZnkxNlBNWOF8qdJqBXBkAGEjc85kTgQCsNSAoN8CPoE7BELAm_EZ68U1Vq/pub?gid=0&single=true&output=csv', {
		download: true,
    header: true,
		config,
...
function calcRating() {
	if (valHead != 0 && valTorso != 0 && valLegs != 0 && valRf != 0 && valLf != 0) {
		...
		if (lang == "eng") {
			$("#scoreBox").text("YOUR ULTIMATE PLAYER RATING: " + playerRating);
		} else {
			$("#scoreBox").text("PUNTAJE DE TU JUGADOR PERFECTO: " + playerRating);
		}
		$("#scoreBox").append("<div class='Stars' style='--rating: " + playerRating + ";' aria-label='star rating'>");
...
function downloadImage() {
	 ...
	 var a = $("#theBody").html()
	 $("#dlCanvas").html(a);
	 $("#dlCanvas .sectHead").hide();
	 $("#dlCanvas #outline").hide();
	 $("#dlCanvas #saveImage").hide();
	 $("#dlCanvas #scoreBox").css("margin-top", "570px");

	 html2canvas(document.querySelector("#dlCanvas")).then(canvas => {

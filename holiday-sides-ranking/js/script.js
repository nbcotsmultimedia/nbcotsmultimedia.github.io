document.addEventListener('DOMContentLoaded', function () {
  ...
  window.checkRanking = function () {
    ...
    if (rankingSubmitted) {
      ...
      $(".actual-order").css("display", "inline-block");

      const actualListItems = [...actualList.children];

      // Add the show-content class to reveal the actual order
      for (let i = 0; i < actualListItems.length; i++) {
        const delay = i * 250;
        setTimeout(() => {
          actualListItems[i].classList.add('show-content');
        }, delay);
        console.log(actualListItems[i])
      } 
      ...
      if (isCorrect) {
        responsiveText.textContent = "Correct! You nailed it!";
      } else {
        responsiveText.textContent = "Oops! Looks like your ranking could use a few adjustments.";
      }

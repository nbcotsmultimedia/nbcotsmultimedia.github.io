...
const urls = {
  nodes:
    "https://docs.google.com/spreadsheets/d/10HP82oK0Sn94Em7W8z2UpsBlFBkFZg_u29YPjRVzQe4/pub?gid=0&single=true&output=csv",
  links:
    "https://docs.google.com/spreadsheets/d/10HP82oK0Sn94Em7W8z2UpsBlFBkFZg_u29YPjRVzQe4/pub?gid=1899076594&single=true&output=csv",
};
...
function updateDetailsPanel1(node) {
...
function updateDetailsPanel(node) {
  ...
  details.append("p").attr("class", "connections-title").html("CONNECTIONS");

  const connections = linksData.filter(
    (link) => link.source === node.id || link.target === node.id
  );
  ...
  connections.forEach((connection) => {
    ...
    const sourceName = sourceNode.name_last || sourceNode.name;
    const targetName = targetNode.name_last || targetNode.name;

    const boldText =
      node.id === connection.source
        ? `${sourceName}'s connection to ${targetName}`
        : `${targetName}'s connection to ${sourceName}`;

    const regularText = `: ${connection.type}`;

    const connectionDetail = details
      .append("p")
      .attr("class", "connection-detail");

    connectionDetail
      .append("span")
      .attr("class", "connection-bold")
      .text(boldText);

    connectionDetail
      .append("span")
      .attr("class", "connection-regular")
      .text(regularText);
  });

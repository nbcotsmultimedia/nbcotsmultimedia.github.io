...
const scrollToSlide = (e, map) => {
    const sections = textContainer.children('section');
    const sectionHeader = $(`h3:contains(${e.target?.options?.title})`)[0];
    ...
    for(let i = 0; i < sections.length; i++) {
        ...
        if (thisSection.id === e.target?.options?.title) {
            sectionHeader.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest'});
            scrolling = true;
...
const addText = (data, map) => {
    ...
    const addInfo = row => {
        ...
        const colHeaders = {
            ...
            "pet": "¿Apto para mascota?",
            "standout": "Vino destacado",
            "reservations": "¿Necesitas reservaciones?",
            "offerings": "Ofertas especiales",
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        + `<p>Número de teléfono: <a href="tel:${row.phone.replace(/-/g, "")}">${row.phone}</a></p>`
...
function loadData() {
	...
	Papa.parse('https://docs.google.com/spreadsheets/d/e/2PACX-1vRmp-X7kZV3NHOVOLgk8REmQYJLwLhvht30B8hfM4J1cmB52RLUT7xQtBWRUyL2CY3CdJHjVVrqlE0V/pub?gid=1785534285&single=true&output=csv', {
		download: true,
    header: true,
		config,

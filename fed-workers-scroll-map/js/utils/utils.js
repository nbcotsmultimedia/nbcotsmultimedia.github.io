...
export function setDimensions(svg) {
...
export function calculateJenksBreaks(data, numClasses) {
  // Safety checks
  ...
  if (validData.length < numClasses) {
    ...
    return calculateQuantileBreaks(validData, numClasses);
  }
  ...
  try {
    ...
    if (sortedData.length > maxSampleSize) {
      ...
      workingData = stratifiedSample(sortedData, maxSampleSize);
      console.log(
    ...
    const breaks = fisherJenks(workingData, numClasses);
...
export function stratifiedSample(sortedData, sampleSize) {
  const result = [];
...
export function fisherJenks(data, numClasses) {
  const n = data.length;
...
export function calculateQuantileBreaks(data, numClasses) {
  if (!data || !Array.isArray(data) || data.length === 0) {
...
export function calculateVariance(array) {
  ...
  const variance = array.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0);

  return variance;
}

// Clean outliers for better visualization
export function cleanOutliers(data, outlierMultiplier = 2.5) {
...
export function identifyOutliers(data) {
...
export function calculatePercentileThresholds(data, percentile = 0.05) {

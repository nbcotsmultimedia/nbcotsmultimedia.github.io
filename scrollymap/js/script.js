...
const addText = (data, map) => {
    ...
    const addInfo = row => {
        requiredCols = ["name", "url", "full_address", "city", "website", "latitude", "longitude", "description", "phone", "ig_name", "ig_username", "image_credit"];
        colHeaders = {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            // if column is image-related, add corresponding html
            switch(thisCol) {
                // if there is an instagram post, embed it
                case "ig_post":
                    if (row.ig_post) {
                        htmlBlock = htmlBlock.concat(`<blockquote class="instagram-media" data-instgrm-captioned
                ...
                case "img":
                    if (row.img) {
                        htmlBlock = htmlBlock.concat(`<img src="./images/${row.img}" alt="${row.name}" width=${mobile ? "100%" : "600"}></img><p>Credit: ${row.image_credit}</p>`);
                    }
                    break;
                // if column is detail-related, add header and info
                default:
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        + `<div class="mobile-text"><h3>${row.name}</h3><p class="address">${row.full_address}</p>`
